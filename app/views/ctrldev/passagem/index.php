        ...
        <div class="form-container" id='formulario'>
            ...
            <form method="post" action="../routes/passagemRoutes.php">
                ...
                <div class="select-all-container mb-2">
                    <input type="checkbox" id="select-all-assentos">
                    <label for="select-all-assentos"><strong>Selecionar todos os assentos</strong></label>
                </div>

                <div class="assentos-grid" id="grid-assentos">
                    <?php if (isset($assentos) && is_array($assentos)): ?>
                        <?php foreach ($assentos as $assento): ?>
                            <div class="assento-card"
                                 data-numero="<?= htmlspecialchars($assento['numero_assento']) ?>"
                                 data-voo="<?= htmlspecialchars($assento['numero_voo']) ?>">
...
<script>
    ...
    document.getElementById('select-all-assentos').addEventListener('change', function () {
        const isChecked = this.checked;
        document.querySelectorAll('.assento-checkbox').forEach(cb => cb.checked = isChecked);
    });
    ...
    filtro.addEventListener('input', function () {
        ...
        assentos.forEach(card => {
            ...
            const voo = card.dataset.voo.toLowerCase();
            card.style.display = (numero.includes(termo) || voo.includes(termo)) ? '' : 'none';
        });
